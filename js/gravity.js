 ...
 * GravityPoint
 */
function GravityPoint(x, y, radius, targets, lifetime) {
    ...
    // Time after which blackholes expire
     setTimeout(() => {
        this.destroyed = true;
    }, lifetime);
}
...
Particle.prototype = (function(o) {
    ...
    if (this.x + this.radius > screenWidth) {
        this.x = screenWidth - this.radius; // Adjust position to stay within bounds
        this._speed.x *= -1; // Reverse horizontal direction
    }
    // Left edge
    else if (this.x - this.radius < 0) {
        this.x = this.radius; // Adjust position to stay within bounds
        this._speed.x *= -1; // Reverse horizontal direction
    }
    // Bottom edge
    if (this.y + this.radius > screenHeight) {
        this.y = screenHeight - this.radius; // Adjust position to stay within bounds
        this._speed.y *= -1; // Reverse vertical direction
    }
    // Top edge
    else if (this.y - this.radius < 0) {
        this.y = this.radius; // Adjust position to stay within bounds
        this._speed.y *= -1; // Reverse vertical direction
    }
